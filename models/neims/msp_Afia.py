...
def main(args):
  ...
  logging.info('Loading weights from %s', args.weights_dir)
  predictor = spectra_predictor.NeimsSpectraPredictor(
      model_checkpoint_dir=args.weights_dir)
  #print("Printing predictor")
  #print(predictor)
  #print(args)
  logging.info('Loading molecules from %s', args.input_file)
  mols_from_file = spectra_predictor.get_mol_list_from_sdf(args.input_file)
  fingerprints, mol_weights = predictor.get_inputs_for_model_from_mol_list(mols_from_file)

  logging.info('Making predictions ...')
  spectra_predictions = predictor.make_spectra_prediction(fingerprints, mol_weights)

  logging.info('Updating molecules in place with predictions.')
  spectra_predictor.update_mols_with_spectra(mols_from_file,spectra_predictions)

  logging.info('Writing predictions to %s', args.output_file)
